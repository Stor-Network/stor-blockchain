...
export default function Block() {
  ...
  const poolReward = storoshi_to_stor(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = storoshi_to_stor(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const storFees = blockRecord.fees
    ? storoshi_to_stor(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.storscan.com/blockchain/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.storscan.com/blockchain/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: storFees ? `${storFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <BlockTitle>
            <Trans>
              Block at height {blockRecord.height} in the Stor blockchain
            </Trans>
          </BlockTitle>
        }
        action={
