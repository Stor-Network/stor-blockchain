...
import storEnvironment from '../util/storEnvironment';
import storConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!storEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    const createWindow = () => {
      ...
      if (!storConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !storConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (storConfig.manageDaemonLifetime()) {
        storEnvironment.startStorDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Stor Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/Stor-Network/stor-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Stor-Network/stor-blockchain/wiki/FAQ',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Stor-Network/stor-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Stor-Network/stor-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Stor-Network/stor-blockchain/issues',
              );
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Visit StorForum' }),
            click: () => {
              openExternal('https://storforum.com');
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Follow on Twitter' }),
            click: () => {
              openExternal('https://twitter.com/stor_network');
            },
          },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // Stor Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Stor' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Stor Blockchain' }),
            click: () => {
              openAbout();
            },
          },
