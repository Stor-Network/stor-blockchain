...
const getStorVersion = () => {
  let version = null;
  const exePath = getExecutablePath('stor');
  // first see if we can get a stor exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or stor is on the path
    try {
      version = child_process
