...
global.daemon_rpc_ws = `wss://${self_hostname}:55600`;
...
function loadConfig(net) {
  try {
    // check if STOR_ROOT is set. it overrides 'net'
    const config_root_dir =
      'STOR_ROOT' in process.env
        ? process.env.STOR_ROOT
        : path.join(os.homedir(), '.stor', net);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 55600); // jshint ignore:line
