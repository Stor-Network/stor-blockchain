...
from stor.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from stor.full_node.full_node_api import FullNodeAPI
from stor.server.start_farmer import service_kwargs_for_farmer
from stor.server.start_full_node import service_kwargs_for_full_node
from stor.server.start_harvester import service_kwargs_for_harvester
from stor.server.start_introducer import service_kwargs_for_introducer
from stor.server.start_service import Service
from stor.server.start_timelord import service_kwargs_for_timelord
from stor.server.start_wallet import service_kwargs_for_wallet
from stor.simulator.start_simulator import service_kwargs_for_full_node_simulator
from stor.timelord.timelord_launcher import kill_processes, spawn_process
...
from stor.util.ints import uint16, uint32
...
async def setup_farmer(
    ...
    config["stor_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "stor")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["stor_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "stor")
