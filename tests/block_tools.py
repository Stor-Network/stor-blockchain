...
from stor.cmds.init_funcs import create_all_ssl, create_default_stor_config
from stor.daemon.keychain_proxy import connect_to_keychain_and_validate, wrap_local_keychain
from stor.full_node.bundle_tools import (
    best_solution_generator_from_template,
    detect_potential_template_generator,
    simple_solution_generator,
)
from stor.util.errors import Err
from stor.full_node.generator import setup_generator_args
from stor.full_node.mempool_check_conditions import GENERATOR_MOD
from stor.plotting.create_plots import create_plots, PlotKeys
...
from stor.consensus.coinbase import create_puzzlehash_for_pk, create_farmer_coin, create_pool_coin
from stor.consensus.condition_costs import ConditionCost
from stor.consensus.constants import ConsensusConstants
from stor.consensus.default_constants import DEFAULT_CONSTANTS
from stor.consensus.deficit import calculate_deficit
from stor.consensus.full_block_to_block_record import block_to_block_record
from stor.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from stor.consensus.pot_iterations import (
    calculate_ip_iters,
...
from stor.consensus.vdf_info_computation import get_signage_point_vdf_info
from stor.full_node.signage_point import SignagePoint
from stor.plotting.util import PlotInfo, PlotsRefreshParameter, PlotRefreshResult, parse_plot_info
from stor.plotting.manager import PlotManager
from stor.server.server import ssl_context_for_server
...
from stor.types.blockchain_format.foliage import Foliage, FoliageBlockData, FoliageTransactionBlock, TransactionsInfo
from stor.types.blockchain_format.pool_target import PoolTarget
from stor.types.blockchain_format.program import INFINITE_COST
from stor.types.blockchain_format.proof_of_space import ProofOfSpace
from stor.types.blockchain_format.reward_chain_block import RewardChainBlockUnfinished
from stor.types.blockchain_format.sized_bytes import bytes32
from stor.types.blockchain_format.slots import (
...
from stor.types.blockchain_format.vdf import VDFInfo, VDFProof
from stor.types.end_of_slot_bundle import EndOfSubSlotBundle
from stor.types.full_block import FullBlock
from stor.types.generator_types import BlockGenerator, CompressorArg
...
from stor.util.block_cache import BlockCache
from stor.util.condition_tools import ConditionOpcode
from stor.util.config import load_config, save_config
from stor.util.hash import std_hash
from stor.util.ints import uint8, uint16, uint32, uint64, uint128
from stor.util.keychain import Keychain, bytes_to_mnemonic
from stor.util.merkle_set import MerkleSet
from stor.util.prev_transaction_block import get_prev_transaction_block
from stor.util.path import mkdir
from stor.util.vdf_prover import get_vdf_info_and_proof
...
class BlockTools:
    ...
    def __init__(
        ...
        create_default_stor_config(root_path)
    ...
    async def setup_keys(self):
        if self.local_keychain:
            ...
            self.keychain_proxy = await connect_to_keychain_and_validate(
                self.root_path, log, user="testing-1.8.0", service="stor-testing-1.8.0"
            )
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `stor generate keys`")
    ...
    async def setup_plots(self):
        ...
        try:
            ...
            plot_keys_2 = PlotKeys(self.farmer_pk, None, encode_puzzle_hash(self.pool_ph, "stor"))
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("STOR_ROOT", "~/.stor/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
