...
from stor.pools.pool_wallet_info import PoolWalletInfo, PoolSingletonState
from stor.protocols import full_node_protocol
from stor.protocols.full_node_protocol import RespondBlock
from stor.rpc.rpc_server import start_rpc_server
from stor.rpc.wallet_rpc_api import WalletRpcApi
from stor.rpc.wallet_rpc_client import WalletRpcClient
from stor.simulator.simulator_protocol import FarmNewBlockProtocol, ReorgProtocol
...
from stor.wallet.derive_keys import master_sk_to_local_sk
...
class TestPoolWalletRpc:
    ...
    async def create_pool_plot(self, p2_singleton_puzzle_hash: bytes32) -> bytes32:
        ...
        try:
            plot_keys = PlotKeys(bt.farmer_pk, None, encode_puzzle_hash(p2_singleton_puzzle_hash, "tstor"))
    ...
    async def test_absorb_self(self, one_wallet_node_and_rpc):
        ...
        tr: TransactionRecord = await client.send_transaction(
            1, 100, encode_puzzle_hash(status.p2_singleton_puzzle_hash, "tstor")
        )
        await time_out_assert(
            10,
            full_node_api.full_node.mempool_manager.get_spendbundle,
    ...
    async def test_absorb_pooling(self, one_wallet_node_and_rpc):
        ...
        # Balance stars at 6 STOR and 5 more blocks are farmed, total 22 STOR
    ...
    async def test_leave_pool(self, setup):
        ...
        try:
            ...
            await time_out_assert(timeout=WAIT_SECS, function=have_stor)
