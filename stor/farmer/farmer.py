...
from stor.consensus.pot_iterations import calculate_sp_interval_iters
from stor.farmer.pooling.og_pool_state import OgPoolState
from stor.farmer.pooling.pool_api_client import PoolApiClient
...
from stor.pools.pool_config import PoolWalletConfig, load_pool_config
from stor.protocols import farmer_protocol, harvester_protocol
from stor.protocols.pool_protocol import (
    ErrorResponse,
...
from stor.server.outbound_message import NodeType, make_msg
from stor.server.server import ssl_context_for_root
from stor.server.ws_connection import WSStorConnection
from stor.ssl.create_ssl import get_mozilla_ca_crt
...
from stor.util.bech32m import decode_puzzle_hash, encode_puzzle_hash
from stor.util.byte_types import hexstr_to_bytes
from stor.util.config import load_config, save_config, config_path_for_filename
from stor.util.hash import std_hash
from stor.util.ints import uint8, uint16, uint32, uint64
...
from stor.wallet.puzzles.singleton_top_layer import SINGLETON_MOD
...
class Farmer:
    ...
    async def setup_keys(self):
        ...
        if len(self.get_public_keys()) == 0:
            error_str = "No keys exist. Please run 'stor keys generate' or open the UI."
            raise RuntimeError(error_str)

        # This is the farmer configuration
        self.farmer_target_encoded = self.config["stor_target_address"]
        self.farmer_target = decode_puzzle_hash(self.farmer_target_encoded)

        self.pool_public_keys = [G1Element.from_bytes(bytes.fromhex(pk)) for pk in self.config["pool_public_keys"]]

        # This is the self pooling configuration, which is only used for original self-pooled plots
        self.pool_target_encoded = self.pool_config["stor_target_address"]
    ...
    def set_reward_targets(self, farmer_target_encoded: Optional[str], pool_target_encoded: Optional[str]):
        ...
        if farmer_target_encoded is not None:
            ...
            config["farmer"]["stor_target_address"] = farmer_target_encoded
        if pool_target_encoded is not None:
            self.pool_target_encoded = pool_target_encoded
            self.pool_target = decode_puzzle_hash(pool_target_encoded)
            config["pool"]["stor_target_address"] = pool_target_encoded
    ...
    async def get_cached_harvesters(self, connection: WSStorConnection) -> HarvesterCacheEntry:
