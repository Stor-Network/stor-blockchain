...
from stor.ssl.create_ssl import generate_ca_signed_cert, get_stor_ca_crt_key, make_ca_cert
...
def check_keys(new_root: Path) -> None:
    ...
    if "stor_target_address" not in config["farmer"]:
        print(f"Setting the stor destination address for coinbase fees reward to {all_targets[0]}")
    ...
    if "stor_target_address" not in config["pool"]:
        print(f"Setting the stor destination address for coinbase reward to {all_targets[0]}")
...
def create_all_ssl(root: Path):
    ...
    stor_ca_crt_path.write_bytes(stor_ca_crt)
    stor_ca_key_path.write_bytes(stor_ca_key)
