...
from stor.daemon.client import acquire_connection_to_daemon
from stor.util.keychain import Keychain, obtain_current_passphrase, supports_os_passphrase_storage
from stor.util.keyring_wrapper import DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE
from stor.util.misc import prompt_yes_no
from stor.util.ws_message import WsRpcMessage
    ...
    try:
        if supports_os_passphrase_storage():
            ...
            print(
                ...
                "Other Stor processes will be able to access your keys without prompting for your passphrase."
            )
...
def initialize_passphrase() -> None:
    if Keychain.has_master_passphrase():
        ...
        print("\nUse 'stor passphrase set' or 'stor passphrase remove' to update or remove your passphrase")
        sys.exit(1)
