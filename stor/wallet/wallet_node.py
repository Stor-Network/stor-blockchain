...
from stor.daemon.keychain_proxy import (
    KeychainProxy,
...
from stor.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from stor.protocols import wallet_protocol
from stor.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
...
from stor.server.node_discovery import WalletPeers
...
from stor.util.byte_types import hexstr_to_bytes
from stor.util.check_fork_next_block import check_fork_next_block
from stor.util.errors import Err, ValidationError
from stor.util.ints import uint32, uint128
from stor.util.keychain import Keychain
from stor.util.lru_cache import LRUCache
from stor.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from stor.util.path import mkdir, path_from_root
from stor.wallet.block_record import HeaderBlockRecord
from stor.wallet.derivation_record import DerivationRecord
from stor.wallet.settings.settings_objects import BackupInitialized
from stor.wallet.transaction_record import TransactionRecord
from stor.wallet.util.backup_utils import open_backup_file
from stor.wallet.util.wallet_types import WalletType
from stor.wallet.wallet_action import WalletAction
from stor.wallet.wallet_blockchain import ReceiveBlockResult
from stor.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'stor keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSStorConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSStorConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSStorConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSStorConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSStorConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSStorConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
...
async def wallet_next_block_check(
    peer: WSStorConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
